...
class GRU(nn.Module):
    def __init__(self, input_size_face, input_size_pose, input_size_smile, input_size_game, hidden_size, num_layers, num_classes, dropout=0.2):
        ...
        self.prediction_layer = nn.Linear(hidden_size, num_classes)
    ...
    def forward(self, face, pose, smile, game):
        ...
        # Late Fusion
        x = torch.cat((x_face, x_pose, x_smile), 1)
    ...
    def _save_model(self, loss, f1, epoch, optimizer):
        if loss < self.best_loss:
            ...
            torch.save({
                        ...
                        }, 'models/MM_GRU/' + wandb.run.name + '.pth')
...
def main(project, entity):
    ...
    with wandb.init(project='Affective_Recognition', entity='iliancorneliussen', config=hyper, tags=["GRU_LF"]):
        config = wandb.config
