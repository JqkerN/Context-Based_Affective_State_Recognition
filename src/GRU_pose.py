...
class GRU(nn.Module):
    ...
    def forward(self, face, pose, smile, game):
        ...
        x, _ = self.gru_pose(pose, h0_pose)  
        x = x[:, -1, :]  
    ...
    def _save_model(self, loss, f1, epoch, optimizer):
        if loss < self.best_loss:
            ...
            torch.save({
                        ...
                        }, 'models/pose/' + wandb.run.name + '.pth')
...
def main(project, entity):
    # imports parameters
    with open("parameters/hyper_params_POSE.yml", "r") as f:
    ...
    with wandb.init(project='Uni-Modal_Affect_Recognition', entity='iliancorneliussen', config=hyper, tags=["pose"]):
        config = wandb.config
